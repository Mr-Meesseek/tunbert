...
## Example
# python models/bert-nvidia/bert_finetuning_QA.py
...
@hydra_runner(config_path="configs", config_name="question_answering_config")
def main(cfg: DictConfig) -> None:
    ...
    for infer_dataset in infer_datasets:
        if infer_dataset.output_prediction_file is not None:
            infer_dataset.output_prediction_file = os.path.join(
                log_dir, infer_dataset.output_prediction_file
            )
        if infer_dataset.output_nbest_file is not None:
            infer_dataset.output_nbest_file = os.path.join(
                log_dir, infer_dataset.output_nbest_file
            )
