...
def main(_):
    ...
    if train_examples:
        num_save_steps = int(len(train_examples) / FLAGS.train_batch_size)
    else:
        num_save_steps = FLAGS.save_checkpoints_steps

    run_config = tf.estimator.tpu.RunConfig(
        cluster=tpu_cluster_resolver,
        master=FLAGS.master,
        model_dir=FLAGS.output_dir,
        save_checkpoints_steps=num_save_steps,
    ...
    if FLAGS.do_eval:
        ...
        eval_model_files = tf.io.gfile.glob(os.path.join(FLAGS.output_dir, "*index"))

        for eval_checkpoint in sorted(
            eval_model_files, key=lambda x: int(x[0:-6].split("-")[-1])
        ):
